...
def test_merge_hook_preserves_media_validation() -> None:
    segment = {
        "order": 1,
    ...
    assert merged["visual_beats"] == [{"order": 1, "phrase_anchor": "Nouveau"}]
    ...
    raw = """```json
{
  ...
  "visual_beats": [{"order": 1, "phrase_anchor": "Question",}],
}
```"""
    data = HookOptimizerAgent._parse_json(raw)
    assert data["narration_text"] == "Question rhétorique ?"
    assert len(data["visual_beats"]) == 1


def test_optimizable_keys_cover_expected_fields() -> None:
    assert "narration_text" in HOOK_OPTIMIZABLE_KEYS
    assert "visual_beats" in HOOK_OPTIMIZABLE_KEYS
