...
const YOUTUBE_OAUTH_BROADCAST = 'cutevideo_youtube_oauth'
...
export default function NewChannelWizardPage() {
  ...
  const [skippedPlatforms, setSkippedPlatforms] = useState<Array<'youtube' | 'tiktok' | 'instagram'>>([])

  const skipPlatform = async (step: 'youtube' | 'tiktok' | 'instagram', nextStep: number) => {
    if (!channelId) return
    setLoading(true)
    setError(null)
    try {
      await skipOnboardingStep(channelId, step)
      setSkippedPlatforms((prev) => (prev.includes(step) ? prev : [...prev, step]))
  ...
  const refreshYoutubeList = useCallback(
    async (forChannelId?: string | null) => {
      const id = forChannelId ?? channelId
      if (!id) return
      setLoading(true)
      setError(null)
      try {
        const list = await listYoutubeChannels(id)
        setYtChannels(list)
        setYoutubeConnected(true)
        if (list.length > 0) {
          setSelectedYtId(list[0].channel_id)
        }
      } catch (e) {
  ...
  useEffect(() => {
    const onOAuthMessage = (payload: {
      type?: string
      status?: string
      channelId?: string
      error?: string
    }) => {
      if (payload.type !== 'youtube_oauth') return
      if (payload.status === 'error') {
        setError(payload.error ?? 'Connexion YouTube échouée')
        return
      }
      ...
      setActiveStep(2)
      void refreshYoutubeList(payload.channelId)
    }

    const bc = new BroadcastChannel(YOUTUBE_OAUTH_BROADCAST)
    bc.onmessage = (event) => onOAuthMessage(event.data)
  ...
  const handleYoutubeDisconnect = async () => {
    ...
    setError(null)
    try {
      await disconnectYoutubeOAuth(channelId)
      setYoutubeConnected(false)
      setYtChannels([])
      setSelectedYtId('')
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Déconnexion YouTube impossible')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <PageContainer maxWidth="md">
      <PageHeader
        title="Créer une chaîne guidée"
        description="Assistant en plusieurs étapes : marché, identité, YouTube, TikTok et Instagram."
        breadcrumbs={[
      ...
      <Stepper activeStep={activeStep} sx={{ mb: 4 }}>
        ...
        {activeStep === 2 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Alert severity="info">
              La connexion YouTube est facultative. Vous pourrez la configurer plus tard dans les paramètres de la
              chaîne.
            </Alert>
            <FormControl>
            ...
            {!youtubeConnected ? (
              <Button variant="outlined" onClick={handleYoutubeOAuth} disabled={loading || !channelId}>
                Connecter Google (OAuth)
              </Button>
            ) : (
              <>
                <Alert severity="success">Compte Google connecté.</Alert>
                <Button
                  variant="outlined"
                  color="error"
            ...
            {youtubeConnected && (
              <Button variant="outlined" onClick={handleRefreshYtList} disabled={loading || !channelId}>
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              ...
              <Button variant="text" onClick={() => skipPlatform('youtube', 3)} disabled={loading || !channelId}>
                Passer cette étape
              </Button>
            </Box>
          </Box>
        ...
        {activeStep === 3 && channelId && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Alert severity="info">
              TikTok est facultatif. Vous pourrez connecter votre compte plus tard depuis la fiche chaîne.
            </Alert>
            <Alert severity="info">
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              ...
              <Button variant="text" onClick={() => skipPlatform('tiktok', 4)} disabled={loading}>
                Passer cette étape
              </Button>
            </Box>
          </Box>
        ...
        {activeStep === 4 && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            <Alert severity="info">
              Instagram est facultatif. Vous pourrez renseigner la Page Business plus tard.
            </Alert>
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              ...
              <Button variant="text" onClick={() => skipPlatform('instagram', 5)} disabled={loading || !channelId}>
                Passer cette étape
              </Button>
            </Box>
          </Box>
        ...
        {activeStep === 5 && brandKit && (
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', my: 1 }}>
              <Chip
                label={
                  skippedPlatforms.includes('youtube')
                    ? 'YouTube : ignoré'
                    : selectedYtId
                      ? 'YouTube : configuré'
                ...
                size="small"
                color={selectedYtId ? 'success' : 'default'}
                variant={skippedPlatforms.includes('youtube') ? 'outlined' : 'filled'}
              />
              <Chip
                label={skippedPlatforms.includes('tiktok') ? 'TikTok : ignoré' : 'TikTok : à connecter plus tard'}
                size="small"
                variant="outlined"
              />
              <Chip
                label={
                  skippedPlatforms.includes('instagram')
                    ? 'Instagram : ignoré'
                    : igPageId.trim()
                      ? 'Instagram : configuré'
