...
export default function ChannelSettingsPage({ params }: Props) {
  ...
  if (isLoading || !form) {
    ...
    return <AppShell><CircularProgress sx={{ m: 4 }} /></AppShell>
  ...
  return (
    ...
    <AppShell>
      ...
      <Box sx={{ maxWidth: 720, mx: 'auto' }}>
        ...
        <Typography variant="h5" sx={{ mb: 1 }}>Paramètres — {channel?.name}</Typography>

        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>

          Thème, format de production, plateformes, voix Azure et générateur d&apos;images IA
        ...
        <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
          ...
          <TextField select fullWidth label="Voix" value={form.tts_voice}

            onChange={(e) => setForm({ ...form, tts_voice: e.target.value })}>

            {(voices || []).map((v: { id: string; label: string }) => (
          ...
          <TextField fullWidth label="Style expressif" value={form.tts_style}
